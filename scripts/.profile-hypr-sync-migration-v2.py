...
def replace_profile_block(path: str, host: str, repo: str) -> None:
    target = Path(path)
    text = target.read_text()
    pattern = re.compile(
        rf'PROFILE="\\${host}"\nwhile IFS= read -r state_file; do\n.*?done < <\\(find "\\$HOME/\\.local/state/nixos-config" -name state\\.json -type f 2>/dev/null \\|\\| true\\)\\)\n',
    ...
    host_ref = f'${host}'
    repo_ref = f'${repo}'
    new = f'''PROFILE="{host_ref}"
PROFILE_FROM_SYSTEM=0
if [[ -r /etc/nixos-config/profile ]]; then
  candidate="$(tr -d '\\r\\n' </etc/nixos-config/profile)"
  case "$candidate" in
    "{host_ref}"|"{host_ref}-mango"|"{host_ref}-niri"|"{host_ref}-hyprland"|"{host_ref}-hyprland-caelestia"|"{host_ref}-mango-niri"|"{host_ref}-mango-hyprland"|"{host_ref}-niri-hyprland"|"{host_ref}-all") PROFILE="$candidate"; PROFILE_FROM_SYSTEM=1 ;;
  esac
fi
if ((PROFILE_FROM_SYSTEM == 0)); then
  while IFS= read -r state_file; do
    ...
    candidate="$(jq -r '.profile // empty' "$state_file" 2>/dev/null || true)"
    case "$candidate" in
      "{host_ref}"|"{host_ref}-mango"|"{host_ref}-niri"|"{host_ref}-hyprland"|"{host_ref}-hyprland-caelestia"|"{host_ref}-mango-niri"|"{host_ref}-mango-hyprland"|"{host_ref}-niri-hyprland"|"{host_ref}-all") PROFILE="$candidate" ;;
    esac
    break
  done < <(find "$HOME/.local/state/nixos-config" -name state.json -type f 2>/dev/null || true)
fi
...
replace_profile_block("scripts/nix-refresh.sh", "HOST", "REPO")
replace_profile_block("scripts/nix-updates.sh", "host", "repo")
